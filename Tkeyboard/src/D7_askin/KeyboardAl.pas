...
type
  ...
  private
    ...
    procedure SetImageDownOffset(const Value: Integer);
  ...
  public
    ...
    property ImageDownOffset:Integer read FImageDownOffset write SetImageDownOffset;
    // methods
  ...
  TKeyControl = class(TsSpeedButton)
  private
    procedure OnDown(Sender: TObject;Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
    procedure OnUp(Sender: TObject;Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
  public
    Key: TKey;
  ...
  public
    ...
    procedure LoadResKeyLayout(ResName: String);
...
procedure TKey.SetImageIndex(const Value: Integer);
begin
FImageIndex:=Value;
if assigned(FControl) then
   FControl.ImageIndex:=Value;
end;
...
var K: TKey; Alphalite:Boolean;
    FontStyle: Integer;
begin
  Reader.ReadListBegin;
  Alphalite:=Reader.ReadInteger=$FFFF;
  if NOT Alphalite then Reader.Position:=Reader.Position-2;
  ...
  begin
    ...
    if Alphalite then K.ImageIndex:=Reader.ReadInteger
                 else K.ImageIndex:=-1;
    FItems.Add(K);
  end;
  Reader.ReadListEnd;
...
begin
  ...
  Writer.WriteInteger($FFFF); //key alphalite
  ...
  begin
    ...
    Writer.WriteInteger(TKey(FItems[i]).ImageIndex);
  end;
  Writer.WriteListEnd;
end;
...
procedure TKeyCollection.LoadFromRes(ResName: String);
VAR Stream: TCustomMemoryStream;
    Reader: TReader;
begin
  // delete all exisiting keys
  DeleteAllKeys;
  Stream:=TResourceStream.Create(hInstance,PChar(ResName),PChar('KLY'));
...
// TKeyboard.Create
//-----------------------------------------------------------------------------
constructor TKeyboardAL.Create(AOwner: TComponent);
begin
...
// TKeyboard.Destroy
//-----------------------------------------------------------------------------
destructor TKeyboardAL.Destroy;
begin
  FKeys.Free;
  inherited;
end;
...
// TKeyboard.GetKeys
//-----------------------------------------------------------------------------
function TKeyboardAL.GetKeys: TKeyCollection;
begin
  Result := FKeys;
end;
...
// TKeyboard.SetKeys
//-----------------------------------------------------------------------------
procedure TKeyboardAL.SetKeys(const Value: TKeyCollection);
begin
  FKeys := Value;
...
//-----------------------------------------------------------------------------
procedure TKeyboardAL.ReCreateControls;
begin
  FreeAllControls;
  CreateControls;
end;
...
//-----------------------------------------------------------------------------
procedure TKeyboardAL.CreateControls;
var
  K: TKeyControl;
  i: Integer;
begin
  ...
  begin
    ...
    K.OnClick := KeyClick;
    K.Images:=FImageList;
    K.ImageIndex:=FKeys[i].ImageIndex;
    if FFlat then K.SkinData.SkinSection:='WEBBUTTON'
             else K.SkinData.SkinSection:='SPEEDBUTTON';
    K.Down:=False;
    FKeys[i].AssignControl(K);
    FKeys[i].ImageDownOffset:=FImageDownOffset;
  end;
end;
...
// Frees all controls.
//-----------------------------------------------------------------------------
procedure TKeyboardAL.FreeAllControls;
var
  i: Integer;
  K: TKeyControl;
begin
...
//-----------------------------------------------------------------------------
procedure TKeyboardAL.ResizeKeyboard;
var
...
begin
  ...
  for i := 0 to FKeys.Count - 1 do
  begin
    FKeys[i].Control.Left := OFFSET+(FKeys[i].Left - MinX) * HORZ_BASE_UNIT + StretchX * (FKeys[i].Left - MinX) div DEFAULT_KEY_WIDTH;
    FKeys[i].Control.Width := FKeys[i].Width * HORZ_BASE_UNIT + round((FKeys[i].Width / DEFAULT_KEY_WIDTH) * StretchX);
    FKeys[i].Control.Top := OFFSET+(FKeys[i].Top - MinY) * VERT_BASE_UNIT + StretchY * (FKeys[i].Top - MinY) div DEFAULT_KEY_HEIGHT;
...
//-----------------------------------------------------------------------------
procedure TKeyboardAL.SetLinkedControl(const Value: TWinControl);
begin
  if Value = Self then
    FLinkedControl := nil
  else
...
//-----------------------------------------------------------------------------
procedure TKeyboardAL.KeyClick(Sender: TObject);
var
  K: TKeyControl;
  V: TKeyValue;
begin
...
//-----------------------------------------------------------------------------
procedure TKeyboardAL.SetShiftState;
var
  i: Integer;
begin
...
//-----------------------------------------------------------------------------
procedure TKeyboardAL.ClearShiftState;
var
  i: Integer;
begin
  for i := 0 to FKeys.Count - 1 do
...
//-----------------------------------------------------------------------------
procedure TKeyboardAL.ToggleCapsState;
begin
  if m_bShiftState then
    ClearShiftState
  else
...
//-----------------------------------------------------------------------------
procedure TKeyboardAL.ToggleAltGrState;
var
  State: TKeyLevel;
  i: Integer;
begin
...
//-----------------------------------------------------------------------------
function TKeyboardAL.GetKeyLevel: TKeyLevel;
var
  Level: TKeyLevel;
begin
  // calculate key level
...
//-----------------------------------------------------------------------------
procedure TKeyboardAL.DoLinkedControlAction(Key: TKey);
var i: Integer;
begin
if not assigned(FLinkedControl) then exit;
case Key.KeyType of
 ktEscape:     begin
                FLinkedControl.Perform(WM_KEYDOWN, VK_ESCAPE, 0);
                FLinkedControl.Perform(WM_KEYUP, VK_ESCAPE, 0);
               end;
 ktEnter:      begin
                FLinkedControl.Perform(WM_KEYDOWN, VK_RETURN, 0);    //WM_CHAR
                FLinkedControl.Perform(WM_KEYUP, VK_RETURN, 0);
               end;
 KtTab:        FLinkedControl.Perform(WM_CHAR, VK_TAB, 0);
 ktBackspace:  begin
                FLinkedControl.Perform(WM_CHAR, VK_BACK, 0);
                FLinkedControl.Perform(WM_KEYDOWN, VK_BACK, 0);
               end;
 ktInsert:     FLinkedControl.Perform(WM_KEYDOWN, VK_INSERT, 0);
 ktDelete:     FLinkedControl.Perform(WM_CHAR, VK_DELETE, 0);
 ...
 ktNormal:     for i:=1 to Length(Key.KeyValue[KeyLevel]) do
                   begin
...
//-----------------------------------------------------------------------------
function TKeyboardAL.SearchKey(KeyCaption: String; KeyLevel: TKeyLevel): TKey;
var
  i: Integer;
begin
  Result := nil;
...
// the first key will be returned.
//-----------------------------------------------------------------------------
function TKeyboardAL.SearchKey(KeyLevel: TKeyLevel; KeyValue: TKeyValue): TKey;
var
  i: Integer;
begin
  Result := nil;
...
// will be returned.
//-----------------------------------------------------------------------------
function TKeyboardAL.SearchKey(KeyType: TKeyType; KeyCaption: String;  KeyLevel: TKeyLevel): TKey;
var
  i: Integer;
begin
  Result := nil;
...
// to the keyboard.
//-----------------------------------------------------------------------------
procedure TKeyboardAL.ReCreateKeys;
begin
  ReCreateControls;
end;
...
//-----------------------------------------------------------------------------
procedure TKeyboardAL.LoadKeyLayout(FileName: String);
begin
...
// TKeyboard.LoadKeyLayout
//
// Loads a new key layout from resurces.
//-----------------------------------------------------------------------------
procedure TKeyboardAL.LoadResKeyLayout(ResName: String);
begin
{  if not FileExists(FileName) then
    raise Exception.Create(Format(ERR_FILE_NOT_FOUND, [FileName]));
 }
...
//-----------------------------------------------------------------------------
procedure TKeyboardAL.SaveKeyLayout(FileName: String);
begin
...
procedure TKeyboardAL.SetImageList(const Value: TCustomImageList);
Var i:Integer;
begin
FImageList:=Value;
for i:=0 to FKeys.Count-1 do
    if FKeys.Items[i].Control<>NIL then
       FKeys.Items[i].Control.Images:=Value;
end;

procedure TKeyboardAL.SetFlat(const Value: Boolean);
Var i:Integer;
begin
FFlat:=Value;
for i:=0 to FKeys.Count-1 do
    if FKeys.Items[i].Control<>NIL then
       if FFlat then FKeys.Items[i].Control.SkinData.SkinSection:='WEBBUTTON'
                else FKeys.Items[i].Control.SkinData.SkinSection:='SPEEDBUTTON';
end;

procedure TKeyboardAL.SetImageDownOffset(const Value: Integer);
Var i:Integer;
begin
FImageDownOffset := Value;
for i:=0 to FKeys.Count-1 do
    if FKeys.Items[i].Control<>NIL then
       FKeys.Items[i].ImageDownOffset:=FImageDownOffset;
end;

{ TKeyControl }

constructor TKeyControl.Create(AOwner: TComponent);
begin
inherited;
OnMouseDown:=onDown;
OnMouseUp:=onUp;
...
procedure TKeyControl.OnDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
ImageIndex:=ImageIndex+Key.FImageDownOffset;
end;

procedure TKeyControl.OnUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
if (ImageIndex-Key.FImageDownOffset)<-1 then exit;
ImageIndex:=ImageIndex-Key.FImageDownOffset;
end;

procedure TKey.SetImageDownOffset(const Value: Integer);
begin
FImageDownOffset:=Value;
end;
