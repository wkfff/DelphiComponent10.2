...
- renamed TBarcode to TBarcode to avoid name conflicts
...
procedure TBarcode.SetWidth(Value :integer);
var
  data : string;
  i : integer;
  w, wtotal : integer;
...
procedure TBarcode.SetTyp(const Value: TBarcodeType);
begin
   if Value <> FTyp then
   begin
      FTyp := Value;
...
procedure TBarcode.SetAngle(const Value: Double);
begin
...
procedure TBarcode.SetText(const Value: string);
begin
   if Value <> FText then
   begin
      FText := Value;
...
procedure TBarcode.SetShowText(const Value: TBarcodeOption);
begin
...
procedure TBarcode.SetTop(const Value: Integer);
begin
   if Value <> FTop then
   begin
      FTop := Value;
...
procedure TBarcode.SetLeft(const Value: Integer);
begin
   if Value <> FLeft then
   begin
      FLeft := Value;
...
procedure TBarcode.SetCheckSum(const Value: Boolean);
begin
...
function TBarcode.GetCanvasHeight: Integer;
var
  alpha :Extended;
begin
  alpha := FAngle/180.0*pi;
...
function TBarcode.GetCanvasWidth: Integer;
var
  alpha :Extended;
begin
  alpha := FAngle/180.0*pi;
...
procedure TBarcode.SetShowTextFont(const Value: TFont);
begin
  FShowTextFont.Assign(Value);
  DoChange;
end;

procedure TBarcode.SetShowTextPosition(const Value: TShowTextPosition);
begin
