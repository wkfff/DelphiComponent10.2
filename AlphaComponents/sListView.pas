...
uses
  ...
  acntTypes, sStyleSimply, acntUtils, sVclUtils, sMaskData, sGraphUtils, sSkinProps, sAlphaGraph, sSkinManager, acAlphaImageList, sThirdParty;
...
{$ENDIF}
    ...
    if FCommonData.Skinned then
      case Message.Msg of
        WM_MOUSEMOVE:
          if (Win32MajorVersion >= 6) and HotTrack then begin
...
var
  ...
  gWidth, ArrowIndex, Flags, State, si, rWidth, i: integer;
    ...
    if (cIndex >= 0) and bool(SendMessage(FHwndHeader, {$IFDEF TNTUNICODE}HDM_GETITEMW{$ELSE}HDM_GETITEM{$ENDIF}, cIndex, LPARAM(@Item))) then begin
      ...
      if ArrowIndex >= 0 then
        ...
        if (ArrowIndex < 0) and (ArrowSide <> asLeft) then begin
          ...
          C := SkinData.SkinManager.gd[si].Props[i].FontColor.Color;
      ...
      if item.iImage >= 0 then begin
        i := mini(State, ac_MaxPropsIndex);
        TempBmp.Canvas.Font.Color := SkinData.SkinManager.gd[si].Props[i].FontColor.Color;
      ...
      if (addr(OnAdvancedCustomDrawItem) = addr(FTempITemAdvancedValue)){ and not (ViewStyle in [vsSmallIcon, vsIcon])} then begin
        ...
        try
          ...
          if SkinData.CustomColor then
            ...
            st := integer(Focused);
            FillDC(DC, ClientRect, SkinData.SkinManager.gd[SkinData.SkinIndex].Props[st].Color);
          end;
  ...
  if (SkinData.SkinManager <> nil) and SkinData.SkinManager.CommonSkinData.Active then begin
    ...
    if Selected then
      ...
      if SkinData.CustomFont then
        ...
        Canvas.Font.Color := ColorToRGB(SkinData.SkinManager.gd[SkinData.SkinIndex].Props[i].FontColor.Color);

    if SkinData.CustomColor then
      Canvas.Brush.Color := Color
    else
      Canvas.Brush.Color := ColorToRGB(SkinData.SkinManager.gd[SkinData.SkinIndex].Props[i].Color);
  end;
end;
...
var
  ...
  begin
    ...
    if (SmallImages <> nil) and (rImage.Left <> rImage.Right) then begin
      if sNdx >= 0 then
        ACanvas.Font.Color := SkinData.SkinManager.gd[sNdx].Props[integer(Focused)].FontColor.Color;
...
{$ENDIF}
    ...
    if Item.ImageIndex >= 0 then begin
      if rImage.Right > rImage.Left then begin
        if sNdx >= 0 then
          Bmp.Canvas.Font.Color := SkinData.SkinManager.gd[sNdx].Props[integer(Focused)].FontColor.Color;
