...
type
  ...
  public
    ...
    procedure DrawCaption(Coloring: TacColoring);
...
uses
  ...
  sThirdParty, acGlow, sAlphaGraph, sMessages, sVclUtils, sMaskData, sMDIForm, acAlphaImageList, acThdTimer;
...
begin
  if DefaultManager <> nil then
    case DefaultManager.Fonts.MainMode of
      ...
      fmFromSkin: Result := DefaultManager.CommonSkinData.MainFont;
    end;
end;
  ...
  if (SkinData <> nil) and SkinData.BGChanged and (CtrlHandle <> 0) then begin
    ...
    if DlgMode and (DefaultManager.Palette[pcBorder] <> clFuchsia) then begin
      ...
      if Assigned(SkinData.SkinManager) and SkinData.SkinManager.IsValidSkinIndex(SkinData.SkinIndex) then
        if (SkinData.FOwnerControl is TCustomEdit) and (TAccessEdit(SkinData.FOwnerControl).BorderStyle = bsNone) then
  ...
  if not IsRectEmpty(bRect) then begin
    ...
    with cd.SkinManager.ConstData.ScrollBtns[Side] do begin
      ...
      if GlyphIndex >= 0 then
        ...
        C := cd.SkinManager.gd[SkinIndex].Props[State].FontColor.Color;
        if (State > 1) and CanClickShift(cd.SkinManager.ConstData.ScrollBtns[Side].SkinIndex, cd.SkinManager) then
          OffsetRect(gRect, 1, 1);

        DrawArrow(b, C, clNone, gRect, Side, 0, 0, 0, cd.SkinManager.Options.ActualArrowStyle);
      end;
    end;
...
begin
  with TNCCalcSizeParams(Pointer(lParam)^) do begin
    ...
    if (sw.skindata.FOwnerControl is TCustomEdit) and not (sw.skindata.FOwnerControl is TCustomMemo) then
      ...
      if not (sw.SkinData.FOwnerObject is TsSkinProvider) and (sw.SkinData.COC <> COC_TsListView) then
...
begin
  ...
  FreeAndnil(sBarVert);
  inherited Destroy;
  if (Ctrl <> nil) {and IsWindowVisible(Ctrl.Handle)} then
          ...
          end;
{
    ...
    case Message.Msg of
      WM_NOTIFY, CN_NOTIFY: Message.Result := CallPrevWndProc(CtrlHandle, Message.Msg, Message.WParam, Message.LParam);
    end;
end;
...
{$ENDIF}
  case Message.Msg of
    ...
    WM_SIZE: begin
      if IsWindowVisible(CtrlHAndle) or (GetWindowLong(CtrlHandle, GWL_STYLE) and WS_VISIBLE <> 0) or (SkinData.FOwnerControl = nil) or not SkinData.FOwnerControl.Visible or (SkinData.COC <> COC_TsAdapterEdit) then // Prevent errors in some controls when not visible
        inherited;
  ...
  if aHandle <> 0 then begin
    if (Repaint or not (Self is TacComboListWnd)) and
      ...
      if ASkinData = nil then begin
        ...
        if not ((ASkinData.COC in [COC_TsListView]) and (Win32MajorVersion >= 6)) then
    ...
    if Assigned(SkinData) and SkinData.Skinned then begin
      if Assigned(SkinData.FOwnerControl) then begin // If VCL used
        ...
        if not SkinData.CustomFont then begin
          ...
          if TacAccessControl(SkinData.FOwnerControl).Font.Color <> C then begin
...
{$ENDIF}
      ...
      if ArrowIndex >= 0 then
        ...
        if ArrowIndex = -1 then begin
          ...
          gR.Bottom := gR.Top + 3;
          State := mini(State, ac_MaxPropsIndex);
          C := SkinData.SkinManager.gd[si].Props[State].FontColor.Color;
          DrawColorArrow(TempBmp, C, gR, ArrowSide);
        end;

      if (State = 2) and CanClickShift(SkinData.SkinIndex, SkinData.SkinManager) then
  ...
  case Message.Msg of
    ...
    WM_PRINT:
      with TWMPaint(Message) do begin
        ...
        if fLeftScrollbar then begin
          OffsX := VSWidth + cxLeftEdge;
          OffsY := cyTopEdge;
        ...
        BitBltBorder(DC, 0, 0, WndSize.cx, WndSize.cy, SkinData.FCacheBmp.Canvas.Handle, 0, 0, cxLeftEdge, cyTopEdge, cxRightEdge, cyBottomEdge);
        Ac_NCDraw(Self, CtrlHandle, -1, DC); // Scrolls painting
        Exit;
      end;
  end;
  inherited acWndProc(Message);
...
begin
  case DefaultManager.Fonts.MainMode of
    ...
    fmFromSkin: sName := DefaultManager.CommonSkinData.MainFont;
  end;
  if sName <> '' then begin
    f := TFont.Create;
    try
...
begin
  ...
  if odSelected in State then begin
    ...
    sNdx := -1;
    C := SkinData.SkinManager.gd[SkinData.SkinIndex].Props[aState].Color;
    ComboBox.Canvas.Font.Color := SkinData.SkinManager.gd[SkinData.SkinIndex].Props[aState].FontColor.Color;
    if CI.Ready then
    ...
    WM_PRINT, WM_PRINTCLIENT:
      if (Message.WParam <> 0) and IsWindowVisible(CtrlHandle) then
        with Message do begin
          ...
          if (Msg <> WM_PRINTCLIENT) and (ProcessMessage(LB_GETCOUNT, 0, 0) >= 1) then begin
            DC := hdc(WParam);
          ...
          if (hpeSortGlyph in Elements) and PaintInfo.ShowSortGlyph then begin
            ...
            if HeaderProp <> nil then begin
              ...
              if Index >= 0 then
                ...
                State := mini(State, ac_MaxPropsIndex);
                C := gd[ConstData.ScrollBtns[Side].SkinIndex].Props[State].FontColor.Color;
                ...
                gR.Bottom := gR.Top + 3;
                gR.Right := gR.Left + 6;
                if (State > 1) and CanClickShift(SkinData.SkinIndex, SkinData.SkinManager) then
                  OffsetRect(gR, 1, 1);
...
begin
  if Caption <> '' then begin
    ...
    LoadCtrlFont(Self);
    Flags := DT_EXPANDTABS or DT_WORDBREAK or DT_NOCLIP or DT_TABSTOP or DT_VCENTER;
    ...
    acWriteTextEx(SkinData.FCacheBmp.Canvas, PacChar(Caption), True, rText, Flags, -1, False);
  end;
  Result := True;
end;
...
function HaveID(CtrlHandle: THandle; aID: integer): boolean;
var
  ChildID: HWND;
begin
  ChildID := aID;
          ...
          if SkinData.Skinned then
            if ID in [idOK, idYes, idCancel, idNo] then begin
              if ID = idCancel then begin
                OkIsFound := HaveID(CtrlHandle, idOK) or HaveID(CtrlHandle, idYes);
                IsBlueCancel := HaveID(CtrlHandle, idNo);
              end
              else begin
                IsBlueCancel := False;
                OkIsFound := True;
...
begin
  ...
  with SkinData do begin
    ...
    if IsWindowEnabled(CtrlHandle) and SkinManager.ButtonsOptions.ShowFocusRect and (ProcessMessage(BM_GETSTATE, 0, 0) and BST_FOCUS <> 0) and (Caption <> '') then begin
      ...
      FocusRect(SkinData.FCacheBMP.Canvas, R, SkinData.SkinManager.gd[SkinData.SkinIndex].Props[State].FontColor.Color, clNone);
    end;
  end;
end;
...
begin
  ...
  if Down and CanClickShift(SkinData.SkinIndex, SkinData.SkinManager) then
    ...
    if OkIsFound then begin
      ...
      if bcGlyph in SkinData.SkinManager.ButtonsOptions.ModalButtonsColoring then
        case ID of
          ...
          idCancel, idNo:
            if IsBlueCancel then
              Coloring.ColorGlyph := iff(CurrentState = 0, slBtnBlueText, slBtnBlueTextActive)
            else
      ...
      if bcText in SkinData.SkinManager.ButtonsOptions.ModalButtonsColoring then
        case ID of
          ...
          idCancel, idNo:
            if IsBlueCancel then
              Coloring.ColorText := iff(CurrentState = 0, slBtnBlueText, slBtnBlueTextActive)
            else
    ...
    with SkinData.SkinManager do begin
      ...
      with ConstData.ScrollBtns[asTop] do
        if GlyphIndex >= 0 then begin
          ...
          R := Btn1Rect;
          if (Btn1State = 2) and CanClickShift(SkinData.SkinIndex, SkinData.SkinManager) then
            OffsetRect(R, 1, 1);

          if sArrowMask >= 0 then begin
            State := math.min(integer(Btn1State > 0), ac_MaxPropsIndex);
            DrawColorArrow(SkinData.FCacheBMP, gd[sArrowMask].Props[State].FontColor.Color, R, asTop)
          end
          else
      ...
      with ConstData.ScrollBtns[asBottom] do
        if GlyphIndex >= 0 then begin
          ...
          R := Btn2Rect;
          if (Btn2State = 2) and CanClickShift(SkinData.SkinIndex, SkinData.SkinManager) then
            OffsetRect(R, 1, 1);

          if sArrowMask >= 0 then begin
            State := math.min(integer(Btn2State > 0), ac_MaxPropsIndex);
            DrawColorArrow(SkinData.FCacheBMP, gd[sArrowMask].Props[State].FontColor.Color, R, asBottom);
          end
          else
    ...
    with SkinData.SkinManager do begin
      ...
      with ConstData.ScrollBtns[asLeft] do
        if GlyphIndex >= 0 then begin
          ...
          if sArrowMask >= 0 then begin
            State := math.min(integer(Btn1State > 0), ac_MaxPropsIndex);
            DrawArrow(SkinData.FCacheBMP, gd[sArrowMask].Props[State].FontColor.Color, clNone, R, asLeft, 0, 0, 0, SkinData.SkinManager.Options.ActualArrowStyle);
          end
          else
            DrawArrow(SkinData.FCacheBMP, 0, clNone, R, asLeft, 0, 0, 0, SkinData.SkinManager.Options.ActualArrowStyle);
        end;
      ...
      with ConstData.ScrollBtns[asRight] do
        if GlyphIndex >= 0 then begin
          ...
          if sArrowMask >= 0 then begin
            State := math.min(integer(Btn2State > 0), ac_MaxPropsIndex);
            DrawArrow(SkinData.FCacheBMP, gd[sArrowMask].Props[State].FontColor.Color, clNone, R, asRight, 0, 0, 0, SkinData.SkinManager.Options.ActualArrowStyle);
          end
          else
            DrawArrow(SkinData.FCacheBMP, 0, clNone, R, asRight, 0, 0, 0, SkinData.SkinManager.Options.ActualArrowStyle);
        end;
    end;
  end;
end;
  ...
  if SkinData.FOwnerControl is TBitBtn then
    with TBitBtn(SkinData.FOwnerControl) do begin
      ...
      if DrawData.Images is TsCharImageList then begin
        C := GetFontColor(SkinData.FOwnerControl, SkinData.SkinIndex, DrawData.SkinManager, DrawData.CurrentState);
...
begin
  ...
  if SkinData.FOwnerControl is TBitBtn then begin
    ...
    if (CurrentState = 2) and CanClickShift(SkinData.SkinIndex, SkinData.SkinManager) then begin
      inc(x);
      inc(y);
    end;
...
begin
  if not HaveImage then
    ...
    if DrawData.Images is TsCharImageList then begin
      C := GetFontColor(Btn, SkinData.SkinIndex, DrawData.SkinManager, DrawData.CurrentState);
      if DrawData.Canvas <> nil then
        DrawData.Canvas.Font.Color := C;
...
var
  ...
  DC, SaveIndex: HDC;
begin
  if not (csDestroying in SkinData.FOwnerControl.ComponentState) and {IsWindowVisible(CtrlHandle) and }not InUpdating(SkinData) then begin
...
var
  ...
  begin
    ...
    i := integer(State <> 0);
    Bmp.Canvas.Font.Color := SkinData.SkinManager.gd[TabIndex].Props[i].FontColor.Color;
  end;
...
{$ENDIF}
    ...
    with SkinData.SkinManager do begin
      if ConstData.Tabs[tlSingle][asTop].SkinIndex > 0 then begin // new style
        TabState := State;
        TabIndex := ConstData.Tabs[tlSingle][acTabSides[TabPosition]].SkinIndex;
        case Style of
          tsTabs:
            with ConstData.Tabs[tlSingle][acTabSides[TabPosition]] do
...
{$IFNDEF FPC}
    if (PageControl <> nil) and TPageControl(SkinData.FOwnerControl).OwnerDraw and Assigned(TPageControl(SkinData.FOwnerControl).OnDrawTab) then begin
      ...
      if TabIndex >= 0 then begin
        ...
        Bmp.Canvas.Font.Color := SkinData.SkinManager.gd[TabIndex].Props[aState].FontColor.Color;
...
var
  ...
  var
    ...
    begin
      ...
      if (Mode = 2) and CanClickShift(SkinData.SkinIndex, SkinData.SkinManager) then
        OffsetRect(Result, 1, 1);
    end;
    ...
    with SkinData.SkinManager, ConstData.ScrollBtns[asBottom] do
      if GlyphIndex >= 0 then begin
        ...
        if (GlyphIndex >= 0) and (GlyphIndex < High(SkinData.SkinManager.ma)) then begin
          ...
          DrawColorArrow(BtnBmp, gd[SkinData.SkinIndex].Props[i].FontColor.Color, gR, asBottom)
        end;
      end;
  end;
...
begin
  with TSpeedButton(Ctrl) do begin
    Size := TextRectSize;
    DoOffset := (CurrentState = 2) and CanClickShift(SkinData.SkinIndex, SkinData.SkinManager);
...
begin
  if (SkinData <> nil) and (SkinData.FOwnerControl <> nil) then
    fLeftScrollbar := SkinData.FOwnerControl.BidiMode = bdRightToLeft
  else
...
begin
  ...
  C := SkinManager.gd[SkinData.SkinIndex].Props[i].FontColor.Color;
