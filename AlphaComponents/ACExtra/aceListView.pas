...
begin
  ...
  if GetCheckBoxesFinal <> gcNone then
    if Assigned(KssListView.StateImages) and (KssListView.StateImages.Count > 5) then begin
      ...
      CheckIndex := KssListView.SkinData.SkinManager.GetMaskIndex(KssListView.SkinData.SkinManager.ConstData.IndexGlobalInfo, s_GlobalInfo, CheckSection);
      ClearRect.Left := WidthOf(KssListView.SkinData.SkinManager.ma[CheckIndex].R) div KssListView.SkinData.SkinManager.ma[CheckIndex].ImageCount;
      ClearRect.Top := HeightOf(KssListView.SkinData.SkinManager.ma[CheckIndex].R) div (KssListView.SkinData.SkinManager.ma[CheckIndex].MaskType + 1);
      FCheckBounds := Rect(OffsetX,
  ...
  if SkinIndex < 0 then begin
    ...
    ci.Y := 0;
    if Selected then
      PaintItem(SkinIndex, ci, true, State, BitmapRect, Point(1, 1), OutCanvas.Handle);
    if Self = KssListView.FHotElement then begin
      BackBitmap := TBitmap.Create;
      BackBitmap.Assign(OutBitmap);
      PaintItem(SkinIndex, ci, true, State, BitmapRect, Point(1, 1), BackBitmap.Canvas.Handle);
    ...
    // Line
    PaintItem(DividerSkinIndex, ci, true, State, LineRect, Point(LineRect.Left, 0), OutCanvas.Handle);
  end;
  ...
  if GetCheckBoxesFinal <> gcNone then
    if Assigned(KssListView.StateImages) and (KssListView.StateImages.Count > 5) then
      ...
      if SkinIndex < 0 then begin
        ...
        DrawSkinGlyph(OutBitmap, Point(FCheckBounds.Left, FCheckBounds.Top), 0, 1, KssListView.SkinData.SkinManager.ma[CheckIndex], MakeCacheInfo(OutBitmap));
...
begin
  ...
  if SkinIndex < 0 then begin
    ...
    ci := MakeCacheInfo(KssListView.FSkinData.FCacheBmp, FBounds.Left, FBounds.Top);
    PaintItem(SkinIndex, ci, true, State, BitmapRect, Point(1, 1), OutBitmap);
    WriteTextEx(OutCanvas, PChar(OutText), true, TxtRec, DT_LEFT, SkinIndex, Boolean(State > 0));
  end;
...
begin
  ...
  begin
    if SkinIndex < 0 then begin
      ...
      PaintItem(SkinIndex, ci, true, 0, BitmapRect, Point(2, 2), OutCanvas.Handle);
    end;
  end;
  ...
  if SkinIndex < 0 then begin
    ...
    if Selected then begin
      ...
      Color.I := clGray;   
      PaintItem(SkinIndex, ci, true, State, BitmapRect, Point(0, 0), BackBitmap.Canvas.Handle);
      if not KssListView.Focused then
        BlendTransRectangle(OutBitmap, 0, 0, BackBitmap, BitmapRect, byte(127))
      else
    ...
    if FShowProgress then begin
      ...
      ci.X := 0;
      ci.Y := 0;
      PaintItem(ProgressSkinIndex, ci, true, State, ClearRect, Point(0, 0), OutCanvas.Handle);
      BackBitmap.Free;
    end;
  ...
  if GetCheckBoxesFinal <> gcNone then
    if Assigned(KssListView.StateImages) and (KssListView.StateImages.Count > 5) then begin
      ...
      DrawSkinGlyph(OutBitmap, Point(FCheckBounds.Left, FCheckBounds.Top), 0 {State (0,1,2)}, 1, KssListView.SkinData.SkinManager.ma[CheckIndex], MakeCacheInfo(OutBitmap));
    end;
...
begin
  ...
  if GetCheckBoxesFinal <> gcNone then
    if Assigned(KssListView.StateImages) and (KssListView.StateImages.Count > 5) then begin
      ...
      CheckIndex := KssListView.SkinData.SkinManager.GetMaskIndex(KssListView.SkinData.SkinManager.ConstData.IndexGlobalInfo, s_GlobalInfo, CheckSection);
      Result := Result + WidthOf(KssListView.SkinData.SkinManager.ma[CheckIndex].R) div KssListView.SkinData.SkinManager.ma[CheckIndex].ImageCount + 5;
    end;
  ...
  if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
    ...
    AC_REFRESH : if (Message.LParam = LPARAM(SkinData.SkinManager)) then begin
      ...
      if FSkinData.Skinned then begin
        if not FSkinData.CustomColor then Color := FSkinData.SkinManager.gd[FSkinData.SkinIndex].Props[0].Color;
        if not FSkinData.CustomFont then Font.Color := FSkinData.SkinManager.gd[FSkinData.SkinIndex].Props[0].FontColor.Color;
      end;
      Repaint;
