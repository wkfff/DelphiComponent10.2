...
procedure TsCustomComboBox.AdjustDropDown;
begin
  inherited;
  inc(SysPopupCount);
end;
...
procedure TsCustomComboBox.CloseUp;
begin
  inherited;
  dec(SysPopupCount);
end;
...
begin
  ...
  if (AState in [1, 3]) and AllowBtnStyle then
    with SkinData.SkinManager, ConstData do
      ShowGlowingIfNeeded(SkinData, AState > 1, Handle, MaxByte * integer(not Effects.AllowAnimation), False, iff(Sections[ssComboNoEdit] >= 0, Sections[ssComboNoEdit], Sections[ssButton]));
...
begin
  if (Style <> csSimple) and FShowButton then
    w := SkinData.SkinManager.ScaleInt(GetComboBtnSize(SkinData.SkinManager) - 1)
  else
    w := 0;
...
function TsCustomComboBox.ColorStored: boolean;
begin
...
{$ENDIF}
  case Message.Msg of
{$IFDEF D2010}
    EM_SETSEL: if csAligning in ControlState then // Solving of system bug with selecting in resizing
      Exit;
{$ENDIF}
  ...
  if not (csDestroying in ComponentState) and FCommonData.Skinned then
    case Message.Msg of
{$IFDEF DELPHI6UP}
      EM_SETSEL: if InUpdating(SkinData) then begin
        SetRedraw(FEditHandle, 0);
        inherited;
        SetRedraw(FEditHandle, 1);
        Exit;
      end;
{$ENDIF}
      WM_MOUSEMOVE:
...
function TsCustomComboBox.FontStored: boolean;
begin
...
begin
  ...
  with FCommonData.SkinManager do
    if not AllowBtnStyle then
      with FCommonData.SkinManager.ConstData.ComboBtn do begin
    ...
    WM_COMMAND, CN_COMMAND:
      if (TWMCommand(Message).NotifyCode = CBN_DROPDOWN) then begin
        ...
        if (SkinData.SkinIndex < 0) and (TWMCommand(Message).NotifyCode = CBN_DROPDOWN) then
          FDropDown := True; // Blinking of label avoiding
      end;
  end;
    ...
    if not (csLoading in ComponentState) then
      case Message.Msg of
        ...
        WM_SIZE, WM_WINDOWPOSCHANGED:
          if FDropDown then // Blinking of label avoiding
            Exit;
      end;
  end
  else begin
...
begin
  ...
  if SkinData.Skinned then begin
    ...
    if (odSelected in State) and not BtnStyle then begin
      ...
      if (sNdx >= 0) {and (SkinData.SkinManager.gd[sNdx].States > 2)} then begin
        C := SkinData.SkinManager.gd[sNdx].Props[2].Color;
        PaintItem(sNdx, CI, True, 1, MkRect(Bmp), MkPoint, Bmp, SkinData.SkinManager);
        Canvas.Brush.Color := C;
        Canvas.Font.Color := SkinData.SkinManager.gd[sNdx].Props[2].FontColor.Color;
      end
      else begin
    ...
    if Assigned(FOnGetItemDrawData) then begin
...
begin
  if M <> D then begin
    inherited;
    if {$IFDEF D2005}Showing{$ELSE}Visible{$ENDIF} then begin
      if not (Style in [csOwnerDrawFixed, csOwnerDrawVariable]) then begin
        i := MulDiv(Height, M, D) - 6;
        SendMessage(Handle, CB_SETITEMHEIGHT, -1, i);
        SendMessage(Handle, CB_SETITEMHEIGHT, 0, i);
      end;
    end;
  end
  else
    ...
    AC_REMOVESKIN:
      if ACUInt(Message.LParam) = ACUInt(SkinData.SkinManager) then begin
        ...
        if not FCommonData.CustomFont then begin
          Font.OnChange := nil; // Don't recreate the control!
