...
uses
  ...
  sFade, sAlphaGraph, sSkinProps, acGlow, acntTypes;
    ...
    if not FGlyph.Empty then begin
      ...
      if FShowGrip then // Grip paint
        if SizingByClick then begin
          ...
          i := min(min(Width, Height) - 2, acArrowSize);
          if SkinData.SkinManager <> nil then
            DrawArrow(Canvas.Handle, ColorToRGB(clBtnText), clNone, GripRect(True), Directions[PrevSize <> 0, Align], 0, 0, i, SkinData.SkinManager.Options.ActualArrowStyle)
          else
            DrawArrow(Canvas.Handle, ColorToRGB(clBtnText), clNone, GripRect(True), Directions[PrevSize <> 0, Align], 0, 0, i, arsSolid1);
        end
        else
      ...
      with SkinData.SkinManager do // Grip paint
        if SizingByClick then begin
          ...
          if ConstData.Sections[ssSpeedButton] >= 0 then begin
            ...
            FreeAndNil(TmpBmp);
            C := gd[ConstData.Sections[ssSpeedButton]].Props[State].FontColor.Color;
          end
          else
            C := 0;
          ...
          if Align in [alTop..alRight] then begin
            i := min(min(Width, Height) - 2, acArrowSize);
            DrawArrow(FCommonData.FCacheBmp, C, clNone, GripRect(True), Directions[PrevSize <> 0, Align], 0, 0, i, SkinData.SkinManager.Options.ActualArrowStyle)
          end;
        end
        else begin
