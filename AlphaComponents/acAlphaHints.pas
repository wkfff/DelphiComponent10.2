...
begin
  ...
  if UseSkinData and Skinned then begin
    ...
    if Result >= 0 then
      ShadowCalculated := DefaultManager.gd[Result].OuterMode > 2;
  end
  else
    Result := -1;
end;
    ...
    if DefaultDraw then
      if Manager.HintFrame <> nil then begin
        ...
        if Manager.Skinned then
          Manager.HintFrame.Font.Color := DefaultManager.gd[SkinIndex].Props[0].Fontcolor.Color
        else
          if (Template <> nil) then
...
{$IFNDEF ACHINTS}
  ...
  PaintItem(SkinIndex, CI, True, 0, MkRect(Self), MkPoint, Result, DefaultManager);
...
{$IFNDEF ACHINTS}
  ...
  if Manager.ShadowCalculated then begin
    ...
    DrawSkinRect32Ex(Result, MkRect(Result), EmptyCI, DefaultManager.ma[DefaultManager.gd[SkinIndex].OuterMask], 0, DefaultManager.gd[SkinIndex].OuterOffset, DefaultManager.gd[SkinIndex].OuterOpacity);

    R.Left := max(0, R.Left + DefaultManager.gd[SkinIndex].OuterOffset.Left);
    R.Top  := max(0, R.Top +  DefaultManager.gd[SkinIndex].OuterOffset.Top);
    R.Right  := min(Width, R.Right - DefaultManager.gd[SkinIndex].OuterOffset.Right);
    R.Bottom := min(Height, R.Bottom - DefaultManager.gd[SkinIndex].OuterOffset.Bottom);

    PaintItem(SkinIndex, CI, True, 0, R{MkRect(Self)}, MkPoint, Result, DefaultManager);
    TextOut(Result);
  end
  else
    if (DefaultManager.gd[SkinIndex].Props[0].Transparency = 100) and (DefaultManager.gd[SkinIndex].BorderIndex >= 0) then
      with DefaultManager.ma[DefaultManager.gd[SkinIndex].BorderIndex] do begin // Used BorderMask
        if Bmp = nil then
        ...
        Mode := integer(DefaultManager.ma[DefaultManager.gd[SkinIndex].BorderIndex].DrawMode and BDM_STRETCH = BDM_STRETCH);
...
begin
{$IFNDEF ACHINTS}
  if DefaultManager.gd[SkinIndex].BorderIndex >= 0 then
    case Border of
      0: begin
        with DefaultManager.ma[DefaultManager.gd[SkinIndex].BorderIndex] do
          if WL > 0 then Result := WL else Result := WidthOf(R) div (ImageCount * 3);

        if Manager.ShadowCalculated then
          inc(Result, DefaultManager.gd[SkinIndex].OuterOffset.Left);
      end;
      1: begin
        with DefaultManager.ma[DefaultManager.gd[SkinIndex].BorderIndex] do
          if WT > 0 then Result := WT else Result := HeightOf(R) div ((MaskType  + 1) * 3);

        if Manager.ShadowCalculated then
          inc(Result, DefaultManager.gd[SkinIndex].OuterOffset.Top);
      end;
      2: begin
        with DefaultManager.ma[DefaultManager.gd[SkinIndex].BorderIndex] do
          if WR > 0 then Result := WR else Result := WidthOf(R)  div (ImageCount * 3);

        if Manager.ShadowCalculated then
          inc(Result, DefaultManager.gd[SkinIndex].OuterOffset.Right);
      end
      else begin
        with DefaultManager.ma[DefaultManager.gd[SkinIndex].BorderIndex] do
          if WB > 0 then Result := WB else Result := HeightOf(R) div ((MaskType  + 1) * 3);

        if Manager.ShadowCalculated then
          inc(Result, DefaultManager.gd[SkinIndex].OuterOffset.Bottom);
      end;
    end
  else
{$ENDIF}
...
begin
  ...
  BodyBmp := GetBody;
  if (DefaultManager.gd[SkinIndex].Props[0].Transparency = 100) and (DefaultManager.gd[SkinIndex].BorderIndex >= 0) or Manager.ShadowCalculated then { BorderMask used }
...
begin
  if SkinIndex >= 0 then
    Result := acLayered and ((DefaultManager.gd[SkinIndex].Props[0].Transparency = 100) and (DefaultManager.gd[SkinIndex].BorderIndex >= 0) or (DefaultManager.gd[SkinIndex].OuterMode > 2) {Custom shadow})
  else
    Result := True;
end;
...
begin
  ...
  if Manager.Skinned and (DefaultManager.gd[CurrentHintForm.SkinIndex].Props[0].Transparency < 100) then
    Transparency := MulDiv(DefaultManager.gd[CurrentHintForm.SkinIndex].Props[0].Transparency, MaxByte, 100)
  else
    Transparency := 0;
