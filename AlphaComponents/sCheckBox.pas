...
begin
  ...
  if FCommonData.Skinned then begin
    if not (csLoading in ComponentState) and AutoSize then begin
      ...
      NewHeight := Max(HeightOf(R), 2 * Margin + ss.cy * integer(Caption <> ''));
      Result := True;
    end;
  end
  else
...
begin
  if Assigned(Images) and (FImgChecked >= 0) and (FImgUnChecked >= 0) then begin
    ...
    if FGlyphChecked.Width > 0 then begin
      ...
      if (FCommonData.SkinManager <> nil) and FCommonData.SkinManager.IsValidImgIndex(FCommonData.SkinManager.ConstData.CheckBox[State]) then
...
begin
  if Assigned(Images) and (FImgChecked >= 0) and (FImgUnChecked >= 0) then begin
    ImgIndex := iff(Checked, FImgChecked, FImgUnChecked);
...
begin
  if Caption <> '' then begin
    ...
    acWriteTextEx(FCommonData.FCacheBmp.Canvas, PacChar(Caption), True, rText, Fmt, FCommonData, PaintCtrlState > 0);
...
begin
  if Assigned(Images) and (FImgChecked >= 0) and (FImgUnChecked >= 0) then
