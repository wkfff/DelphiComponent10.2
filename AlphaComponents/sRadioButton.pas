...
begin
  if FGlyphChecked.Width > 0 then begin
    ...
    if (FCommonData.SkinManager <> nil) and FCommonData.SkinManager.IsValidImgIndex(FCommonData.SkinManager.ConstData.RadioButton[Checked]) then
...
{$ENDIF}
  ...
  if (FCommonData <> nil) and FCommonData.Skinned(True) then
    case Message.Msg of
      ...
      CM_MOUSELEAVE:
        if Enabled and not (csDesigning in ComponentState) and Visible then begin
