...
begin
  ...
  R := PageRect;
  PaintItem(SkinData.SkinIndex, CI, False, 0, R, Point(Left + R.Left, Top + R.Top), SkinData.FCacheBmp, SkinData.SkinManager);
...
var
  ...
  begin
    ...
    st := integer(State > 0);
    Bmp.Canvas.Font.Color := SkinData.SkinManager.gd[TabSkinIndex].Props[st].FontColor.Color;
  end;
      ...
      AC_ENDPARENTUPDATE:
        if SkinData.Skinned then
          if FCommonData.FUpdating then begin
            if InUpdating(FCommonData, True) then
