...
{$ENDIF}
  ...
  public
    RTInit,
    RTEmpty, // SkinProvider is exists already
    InAero,
    InMenu,
    AllowScale,
    biClicked,
...
begin
  ...
  SetWindowPos(Result.Handle, HWND_TOPMOST, R.Left{ + 100}, R.Top, 0, 0, SWPA_SHOWZORDER);
  FreeAndNil(Bmp);
end;
...
begin
  Result := False;
  if (acWinVer < 6) or AeroIsEnabled then begin
...
begin
  ...
  if Result = 0 then begin
    ...
    if Style and WS_THICKFRAME <> 0 then
      if (BorderForm = nil) or (BorderForm.SkinData.SkinManager = nil) then
...
{$ENDIF}
  case Message.Msg of
    WM_PAINT: begin
      ...
      try
        if SP.SkinData.FCacheBmp <> nil then
          if SP.BorderForm <> nil then // If paint menu only
            if IsZoomed(SP.Form.Handle) then
              BitBlt(DC, 0, 0, Width, Height, SP.SkinData.FCacheBmp.Canvas.Handle, SP.OffsetX - SP.Form.BorderWidth, SP.OffsetY - SP.LinesCount * SP.MenuHeight - SP.Form.BorderWidth - 1 + integer(GetWindowLong(SP.Form.Handle, GWL_STYLE) and WS_CAPTION <> 0) * 4, SRCCOPY)
            else
              BitBlt(DC, 0, 0, Width, Height, SP.SkinData.FCacheBmp.Canvas.Handle, SP.OffsetX - SP.Form.BorderWidth, SP.OffsetY - SP.LinesCount * SP.MenuHeight - SP.Form.BorderWidth - 1, SRCCOPY)
          else
...
begin
  with sp do
    if CanDrawNCArea and not InAnimation(sp) and (Form.WindowState <> wsMinimized) and Form.Showing then
...
begin
  with sp.Form do
    Result := (BorderStyle <> bsNone) or (GetWindowLong(Handle, GWL_STYLE) and WS_CHILD <> 0)
end;
...
begin
  with sp, Form do
    if CanDrawNCArea and HandleAllocated and not InMenu and (HaveBorder(sp) or IsIconic(Handle) or IsSizeBox(Handle)) then begin
...
{$IFNDEF NOFONTSCALEPATCH}
          if (SkinData.SkinManager.SysFontScale > 0) and (SkinData.SkinManager.Options.ScaleMode = smOldMode) then
            rgn := CreateRectRgn(sbw, i - 1, Width - sbw, Height - sbw)
          else
{$ENDIF}
...
begin
  ...
  if sp.SkinData.SkinManager <> nil then
    Result := sp.SkinData.SkinManager.ScaleInt(Result);
end;
    ...
    if IconVisible then begin
      ...
      if not Glyph.Empty then begin
        ...
        if SkinData.SkinManager <> nil then begin
          iW := SkinProvider.SkinData.SkinManager.ScaleInt(iW);
          iH := SkinProvider.SkinData.SkinManager.ScaleInt(iH);
        end;
...
begin
{$IFDEF LOGGED}
  if (Tag = 1) {and (Form.WindowState = wsMaximized)} then
    AddToLog(Message);
{$ENDIF}
...
begin
  if (i <> CurrentHT) and CanDrawNCArea then begin
...
{$ENDIF}
  ...
  begin
    if (iCaptHeight <> 0) and CanDrawNCArea then
  ...
  begin
    if (iCaptHeight <> 0) and CanDrawNCArea then begin // Out the title text
      ...
      if Assigned(FTitleBar) then begin
        ...
        if CaptionAlignment <> taCenter then begin
          if Form.BidiMode = bdRightToLeft then begin
            R.Right := SkinData.FCacheBmp.Width - (SysBorderWidth(Form.Handle, BorderForm) + integer(IconVisible) * WidthOf(FIconRect) + 4 +
                      sMan.CommonSkinData.BIRightMargin + aShadowSize.Right + integer(FCaptionSkinIndex >= 0) * 2);

            R.Left := TitleBtnsWidth {- 12} + aShadowSize.Right + SysBorderWidth(Form.Handle, BorderForm) + acSpacing;
          end
          else begin
            R.Left := SysBorderWidth(Form.Handle, BorderForm) + integer(IconVisible) * WidthOf(FIconRect) + 4 +
                      sMan.CommonSkinData.BILeftMargin + aShadowSize.Left + integer(FCaptionSkinIndex >= 0) * 2;

            R.Right := fWidth - TitleBtnsWidth {- 12} - aShadowSize.Right - SysBorderWidth(Form.Handle, BorderForm) - acSpacing;
          end;
        end
        else begin
      ...
      if not IsRectEmpty(R) then begin
          ...
          case DefaultManager.Fonts.MainMode of
            ...
            fmFromSkin:
              if DefaultManager.CommonSkinData.MainFont <> '' then
                FCommonData.FCacheBmp.Canvas.Font.Name := DefaultManager.CommonSkinData.MainFont;
          end;
        ...
        case TextAlign of
          Classes.taCenter: begin
            R.Left := max(R.Left, R.Left + (WidthOf(R) - ts.cx - tsAdded.cx - acSpacing) div 2);
            R.Right := min(R.Left + ts.cx, R.Right);
          end;
          Classes.taRightJustify: begin
            i := ts.cx + tsAdded.cx + acSpacing;
            if i < WidthOf(R) then
              R.Left := R.Right - i;
          end;
        end;
        ...
        if ShowCaption then begin
          ...
          if sIndex >= 0 then begin
            ...
            i := sMan.gd[sIndex].Props[i].GlowSize;
            if i <> 0 then
              if FormActive then
          ...
          if (R.Left + 8 < R.Right) and (FTitleSkinIndex >= 0) then begin
            ...
            case FAddedTitle.Font.Color of
              clNone, clWindowText: begin
                ...
                FCommonData.FCacheBmp.Canvas.Font.Color := sMan.gd[FTitleSkinIndex].Props[st].FontColor.Color;
              end
              else
            ...
            if BorderForm = nil then begin
              ...
              WriteText32(FCommonData.FCacheBmp, PacChar(FAddedTitle.Text), True, R, Flags, -1, integer(FormActive), sMan);
          end;
        end;
      end;
    end;
...
{$ENDIF}
      ...
      if (iCaptHeight <> 0) and CanDrawNCArea then begin
...
begin
  with TempBmp do begin
    PixelFormat := pf32bit;//FCommonData.FCacheBmp.PixelFormat;
...
{$IFNDEF NOFONTSCALEPATCH}
          if (SkinData.SkinManager.SysFontScale > 0) and (SkinData.SkinManager.Options.ScaleMode = smOldMode) then begin
            dec(y, 1);
            inc(ty, 2);
          end;
{$ENDIF}
...
begin
  if FCommonData.Skinned and not SkinData.CustomColor then begin
    i := integer(FormActive);
    Result := FCommonData.SkinManager.gd[FCommonData.Skinindex].Props[i].Color;
  end
  else
...
begin
  ...
  if (SkinData.SkinManager <> nil) and SkinData.SkinManager.CommonSkinData.Active and Form.HandleAllocated then
  	SetClassLong(Form.Handle, GCL_HBRBACKGROUND, Integer(SkinData.SkinManager.Brushes[pcMainColor]));
end;
...
var
  ...
  begin
    ...
    if Form.BidiMode = bdRightToLeft then begin
      ...
      // New right Offset
      FBarRect.Left := Btn.cpRect.Right + integer(BigButtons) * FCommonData.SkinManager.CommonSkinData.BISpacing;
    end
    else begin
...
//        dec(FullLeft, min(0, DiffBorder(BorderForm) - 2));
    end
    else begin
...
//        inc(FullRight, min(0, DiffBorder(BorderForm) - 2));
    ...
    if FIconRect.Right <> FIconRect.Left then begin
      i := TitleIconHeight(Self);
    ...
    if Assigned(SystemMenu) and HaveSysMenu then begin
      ...
      if biHelp in Form.BorderIcons then begin
        ...
        if Form.BidiMode = bdRightToLeft then
          inc(FBarRect.Left, acSpacing)
        else
          dec(FBarRect.Right, acSpacing);
    ...
    if TitleButtons.Count > 0 then begin
      ...
      if Form.BidiMode = bdRightToLeft then
        FBarRect.Left := FBarRect.Left + UserButtonsOffset
      else
        FBarRect.Right := FBarRect.Right - UserButtonsOffset;
    ...
    if Assigned(FTitleBar) then begin
      ...
      if Form.BidiMode = bdRightToLeft then begin
        ...
        inc(Bar.FBarRect.Left, UserButtonsOffset); // Spacing between sys buttons and others
      end
      else begin
        if FIconRect.Right <> FIconRect.Left then
          for i := 0 to Bar.Items.Count - 1 do
            if Bar.Items[i].Visible and (Bar.Items[i].Align = tbaRight) then begin
              inc(Bar.FBarRect.Left, UserButtonsOffset);
              Break;
            end;
...
var
  ...
  begin
    ...
    if (Item.Style in [bsMenu, bsButton]) and (Item.DropDownMenu <> nil) then begin
      ArrowSize := acArrowSize * 2;
      aSize := ArrowSize + acSpacing;
    end
    else begin
      ArrowSize := 0;
      aSize := 0;
    ...
    case Item.Alignment of
      ...
      taCenter: begin
        ...
        if cSize > 0 then
          inc(cSize, Item.Spacing * integer(ts.cx > 0));

        rGlyph.Left := R.Left + (WidthOf(R) - cSize - ts.cx - aSize) div 2;
      end;
      ...
      taRightJustify: begin
        ...
        rGlyph.Left := R.Left + (Item.Width - cSize - ts.cx - ArrowSize - acSpacing * 2);
      end;
    end;

    ParentFont := False;
    if (iState = 0) and (sIndex >= 0) and Item.FontData.UseSysColor then
      if (SkinData.SkinManager.gd[sIndex].BorderIndex < 0) and (SkinData.SkinManager.gd[sIndex].Props[iState].Transparency > 50) and
            (SkinData.SkinManager.ConstData.Sections[ssFormTitle] >= 0) then begin
        ParentFont := True;
        DstBmp.Canvas.Font.Color := SkinData.SkinManager.gd[SkinData.SkinManager.ConstData.Sections[ssFormTitle]].Props[integer(FormActive)].FontColor.Color;
      end;

    if Assigned(FTitleBar.Images) and IsValidIndex(ImgNdx, GetImageCount(FTitleBar.Images)) then
      GlyphSize := MkSize(FTitleBar.Images.Width, FTitleBar.Images.Height)
    else
      if Assigned(Item.Glyph) and not Item.Glyph.Empty then
        GlyphSize := MkSize(Item.Glyph.Width, Item.Glyph.Height)
      else
        GlyphSize := MkSize;

    // Glyph rect
    rGlyph.Top := R.Top + (Item.Height - GlyphSize.cy) div 2;
    ...
    if (GlyphSize.cx <> 0) and (Item.Caption <> '') then
      inc(rText.Left, Item.Spacing);

    if Item.Caption <> '' then begin
      Flags := DT_SINGLELINE or DT_END_ELLIPSIS or DT_TOP;
      rText.Top := R.Top + (Item.Height - ts.cy) div 2;
      rText.Bottom := rText.Top + ts.cy;
      rText.Right := rText.Left + ts.cx;
    end
    else
    ...
    if ArrowSize <> 0 then begin
      ...
      if (rText.Right <> rText.Left) or (rGlyph.Right <> rGlyph.Left) then
        inc(rArrow.Left, acSpacing);

      rArrow.Top := R.Top;
      rArrow.Bottom := R.Bottom;
      rArrow.Right := rArrow.Left + ArrowSize;
    end
    else
    ...
    if Assigned(FTitleBar.Images) and IsValidIndex(ImgNdx, GetImageCount(FTitleBar.Images)) then
      if FTitleBar.Images is TsAlphaImageList then
        DrawAlphaImgList(FTitleBar.Images, DstBmp, rGlyph.Left, rGlyph.Top, ImgNdx, 0, C, 0, 1, False)
      else
      ...
      if Assigned(Item.Glyph) and not Item.Glyph.Empty then begin
        ...
        if Item.Glyph.PixelFormat = pf32bit then
          ...
          DstBmp.Canvas.Draw(rGlyph.Left, rGlyph.Top, Item.Glyph);
      end;
    ...
    if Item.Caption <> '' then begin
      ...
      WriteText32(DstBmp, PacChar(s), Form.Enabled, rText, Flags, TxtNdx, iState, sMan);
    end
    else
      rText.Right := rText.Left;
    ...
    if ArrowSize <> 0 then
      if ParentFont then
        DrawArrow(DstBmp, SkinData.SkinManager.gd[SkinData.SkinManager.ConstData.Sections[ssFormTitle]].Props[integer(FormActive)].FontColor.Color, clNone, rArrow, asBottom, 0, 0, 0, SkinData.SkinManager.Options.ActualArrowStyle)
      else
        if Item.FontData.UseSysColor and (sIndex >= 0) then
          DrawArrow(DstBmp, SkinData.SkinManager.gd[sIndex].Props[iState].FontColor.Color, clNone, rArrow, asBottom, 0, 0, 0, SkinData.SkinManager.Options.ActualArrowStyle)
        else
          DrawArrow(DstBmp, DstBmp.Canvas.Font.Color, clNone, rArrow, asBottom, 0, 0, 0, SkinData.SkinManager.Options.ActualArrowStyle)
  end;
      ...
      case Item.Style of
        bsMenu: begin
          if sIndex < 0 then begin
            ...
            PaintItemSection(sIndex, MakeCacheInfo(SkinData.FCacheBmp), True, Item.State, bR, Item.Rect.TopLeft, DstBmp, sMan);
        end;
        ...
        bsTab: begin
          ...
          if sIndex >= 0 then begin
            ...
            if Item.FontData.UseSysGlowing then begin
              ...
              gSize  := sMan.gd[sIndex].Props[i].GlowSize;
              gColor := sMan.gd[sIndex].Props[i].GlowColor;
            end
          end;
          // Align content to center
          bR.Left := bR.Left + (WidthOf(bR) - Item.ContentSize.cx) div 2;
        ...
        bsInfo, bsButton: begin
          ...
          if (Item.SkinSection = '') and (Item.State = 0) then begin // Get font color from title if button is not active
            ...
            if Item.FontData.UseSysColor and IsValidIndex(FTitleSkinIndex, Length(sMan.gd)) then
              DstBmp.Canvas.Font.Color := sMan.gd[FTitleSkinIndex].Props[iState].FontColor.Color
            else
              DstBmp.Canvas.Font.Color := Item.FontData.Font.Color;

            if Item.FontData.UseSysGlowing and IsValidIndex(FTitleSkinIndex, Length(sMan.gd)) then begin
              gSize  := sMan.gd[FTitleSkinIndex].Props[iState].GlowSize;
              gColor := sMan.gd[FTitleSkinIndex].Props[iState].GlowColor;
            end
            else begin
              gSize  := 0;
              gColor := 0;
  ...
  begin
    ...
    if not Defined then
      case FTitleBar.Items[i].Style of
        bsButton: isd^.SkinIndex := SkinData.SkinManager.ConstData.Sections[ssTBBTN];
        bsMenu:   isd^.SkinIndex := SkinData.SkinManager.ConstData.Sections[ssTBMENUBTN];
        bsTab:    isd^.SkinIndex := SkinData.SkinManager.ConstData.Sections[ssTBTAB];
      end;
  end;
  ...
  begin
    ...
    if (acIntController <> nil) and (Length(acIntController.FormHandlers) > 0) then
      for i := 0 to Length(acIntController.FormHandlers) - 1 do
        if acIntController.FormHandlers[i] <> nil then
          with acIntController.FormHandlers[i] do
            if not acIntController.FormHandlers[i].Closed and (ParentForm = Form) and (PopupForm <> nil) and (PopupCtrl <> nil) then begin
              Result := True;
              break;
            end;
  end;
...
{$ENDIF}
        ...
        b := Form.BorderStyle = bsNone;
      end
      else
        b := False;

      if b or HeaderUsed then
        SetParentUpdated(Form);
    end;
  end;
...
//            FCommonData.BGChanged := True; // Clear cache if form closing cancelled
              SetHotHT(0);
            end;
        end;
...
begin
  if TitleItem.ShowHint and (TitleItem.Hint <> '') and Form.ShowHint and Application.ShowHint then begin
    if FHintTimer = nil then
      ...
      if not InAnimationProcess then
        if Form.Showing and (Form.WindowState <> wsMinimized) then begin
          if (FCommonData.BGChanged or FormChanged) and not (IsIconic(Form.Handle) and InAero) then begin
            ...
            if CanDrawNCArea then begin
              if SkinData.SkinManager.Options.NativeBordersMaximized and (Form.WindowState = wsMaximized) and (BorderForm <> nil) then //begin
                FreeAndNil(BorderForm);
//                SetWindowPos(Form.Handle, 0, 0, 0, 0, 0, SWPA_FRAMECHANGED);
//              end;
...
{$ENDIF}
          ...
          SetClassLong(Form.Handle, GCL_HBRBACKGROUND, Integer(GetSysColorBrush(COLOR_BTNFACE)));
  ...
  case Message.WParamHi of
    ...
    AC_GETFONTINDEX:
      if (FormHeader.AdditionalHeight > 0) and (FHeaderIndex >= 0) and (PacPaintInfo(Message.LParam).R.Top < FormHeader.AdditionalHeight) then begin
        ...
        if SkinData.SkinManager.CommonSkinData.Version >= 10.25 then begin
          if SkinData.SkinManager.gd[SkinData.SkinIndex].States <= PacPaintInfo(Message.LParam)^.State then
...
{$ENDIF}
    ...
    AC_CHILDCHANGED: with FCommonData, SkinManager do begin
...
{$ENDIF}
    ...
    AC_PREPARECACHE:
      if not aSkinChanging and FCommonData.BGChanged and not FCommonData.FUpdating then begin
        PaintAll;
...
var
  i: integer;
begin
  if (Message.WParam = 1) and AllowScale and Assigned(SkinData.SkinManager) {$IFDEF D2010}and (Form.ClassName <> 'TMessageForm'){$ENDIF} then
    SkinData.SkinManager.UpdateScale(Form);
           ...
           not IsIconic(Form.Handle) and
             not SkipAnimation and
               SkinData.SkinManager.Active and
                 CanDrawNCArea and
                   SkinData.SkinManager.Effects.AllowAnimation and
                     SkinData.SkinManager.AnimEffects.FormHide.Active then begin
        acHideTimer := nil;
        SkipAnimation := True;
        AnimHideForm(Self);
    ...
    fsMDIForm: begin
      ...
      case Message.WParam of
        SC_MAXIMIZE, SC_RESTORE, SC_MINIMIZE: begin
          ...
          case Message.WParam of
            SC_MAXIMIZE: if BorderForm <> nil then
              BorderForm.UpdateExBordersPos(True);
        ...
        if (Form.FormStyle = fsMDIForm) and (Form.ActiveMDIChild <> nil) and (Form.ActiveMDIChild.WindowState = wsMaximized) then begin
          ...
          if not FCommonData.FUpdating then begin
            if (FCommonData.FCacheBmp <> nil) and not FCommonData.BGChanged then
              CopyWinControlCache(Form, FCommonData, Rect(OffsetX, OffsetY, OffsetX + Form.ClientWidth, OffsetY + Form.ClientHeight), MkRect(Form.ClientWidth, Form.ClientHeight), hdc(TWMPaint(Message).DC), False);
        ...
        if not IsIconic(Form.Handle) then
          if (SkinData.SkinManager.ShowState = saMinimize) and (Form = Application.MainForm) then begin
            ...
            if SkinData.SkinManager.ShowState <> saMinimize then begin // Closing
              if FAllowAnimation and SkinData.SkinManager.AnimEffects.FormHide.Active and SkinData.SkinManager.Effects.AllowAnimation and (Form.Parent = nil) and (Form.FormStyle <> fsMDIChild) then begin
                if AeroIsEnabled then
...
begin
  if Message.Control <> nil then begin
    OldWndProc(TMessage(Message));
{
    if (Message.Control is TCustomForm) and (Message.Control.Tag and ExceptTag <> ExceptTag) then begin
      if Form.Perform(SM_ALPHACMD, AC_GETPROVIDER_HI, 0) <> 0 then begin
        ap := TacProvider.Create(Message.Control);
        acSupportedList.add(ap);
        ap.InitForm(TCustomForm(Message.Control));
      ...
      if Control is TFrame then
        ...
        if Control is TForm then
          ...
          if Provider.OwnThirdLists then
            SearchSupport(Provider.ThirdLists, False, Result)
          else
            SearchSupport(Provider.SkinData.SkinManager.ThirdLists, False, Result);
    end
    else
      if Control is TCustomLabel then
        Result := CheckSkinEvent
        ...
        if Control is TSpeedButton then
          ...
          if Provider.OwnThirdLists then
            SearchSupport(Provider.ThirdLists, True, Result)
          else
            SearchSupport(Provider.SkinData.SkinManager.ThirdLists, True, Result);
end;
...
{$ENDIF}
    ...
    WM_WINDOWPOSCHANGING:
      if (FOwner is TsSkinProvider) and (FormState and FS_BLENDMOVING <> 0) then begin
...
{$ENDIF}
        if SkinData.SkinManager.Effects.AllowAeroBluring and SkinData.SkinManager.CommonSkinData.UseAeroBluring and AeroIsEnabled then begin
          MakeBluringRgn;
...
{$IFDEF DELPHI6UP}
  if not Form.Enabled then begin
    ...
    C.R := (C.R * (MaxByte - FDisabledBlendValue)) shr 8;
    C.G := (C.G * (MaxByte - FDisabledBlendValue)) shr 8;
    C.B := (C.B * (MaxByte - FDisabledBlendValue)) shr 8;

    FillRect32(AlphaBmp, MkRect(AlphaBmp), C.C, MaxByte - FDisabledBlendValue);

    FBmpSize := MkSize(AlphaBmp);
    FBmpTopLeft := MkPoint;
    InitBlendData(FBlend, MaxByte);

    if FDisabledBlured then
