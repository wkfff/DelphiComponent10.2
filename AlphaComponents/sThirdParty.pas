...
function GetImageWidth(ImgList: TCustomImageList; ImageIndex: integer = -1): integer;
function GetImageHeight(ImgList: TCustomImageList; ImageIndex: integer = -1): integer;
procedure DrawBtnGlyph(Button: TControl; Canvas: TCanvas; ColorTone: TColor);// = clNone);// = nil);
  ...
  if ImgList <> nil then
    if ImgList is TsAlphaImageList then
      ...
      if ImgList is TsCharImageList then begin
        cil := TsCharImageList(ImgList);
        Result := cil.Width;
        if IsValidIndex(ImageIndex, cil.Count) then
          Result := Round(Result * cil.Items[ImageIndex].ScalingFactor);
      end
      else
        if (ImgList is TsVirtualImageList) and (TsVirtualImageList(ImgList).AlphaImageList is TsCharImageList) then begin
          vil := TsVirtualImageList(ImgList);
          cil := TsCharImageList(TsCharImageList(vil.AlphaImageList));
          Result := vil.Width;
          if IsValidIndex(ImageIndex, cil.Count) then
            Result := Round(Result * cil.Items[ImageIndex].ScalingFactor);
        end
...
var
  ...
  begin
    ...
    if (DrawData.Images is TsCharImageList) or (DrawData.Images is TsVirtualImageList) then
      if SkinData.SkinIndex >= 0 then
        DrawData.Canvas.Font.Color := GetFontColor(Button, SkinData.SkinIndex, DrawData.SkinManager, DrawData.CurrentState, SkinData)
      else
...
{$ENDIF}
    ...
    if (sd <> nil) and sd.Skinned then begin
      ...
      if (DrawData.CurrentState = 0) and
        ...
        DrawData.SkinIndex := GetFontIndex(ToolBar, DrawData.SkinManager.ConstData.Sections[ssToolButton], sd.SkinManager)
      else
        DrawData.SkinIndex := DrawData.SkinManager.ConstData.Sections[ssToolButton];//sd.SkinIndex;
    end
    else begin
      DrawData.Grayed      := False;
...
{$ENDIF}
    ...
    if (DrawData.Images is TsCharImageList) or (DrawData.Images is TsVirtualImageList) then begin
      C := GetFontColor(nil{Button}, DrawData.SkinIndex, DrawData.SkinManager, DrawData.CurrentState);
  ...
  if (DrawData.CurrentState = 2) and CanClickShift(DrawData.SkinIndex, DrawData.SkinManager) then
