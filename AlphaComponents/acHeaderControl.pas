...
const
  ...
  Spacing = 10;
var
  i, j, si, Ndx, ImgW, Index, State: integer;
...
{$ENDIF}
      ...
      if (Length(Section.Text) > 0) or (ImgW > 0) then begin
        if (State = 2) and not CanClickShift(SkinData.SkinIndex, SkinData.SkinManager) then
          State := 1;

        if ImgW > 0 then begin
          j := mini(State, ac_MaxPropsIndex);
          TempBmp.Canvas.Font.Color := SkinData.SkinManager.gd[si].Props[j].FontColor.Color;
