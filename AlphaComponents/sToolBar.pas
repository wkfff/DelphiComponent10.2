...
{$WARNINGS ON}
  ...
  begin
    i := integer(Mode > 0);
    BtnBmp.Canvas.Font.Color := FCommonData.SkinManager.gd[iSkinIndex].Props[i].FontColor.Color;
    ...
    with FCommonData.SkinManager, gd[iSkinIndex] do begin
      ...
      with ConstData.ScrollBtns[asBottom] do
        if IsValidIndex(GlyphIndex, High(ma)) then begin
          ...
          i := integer(Mode > 0);
          DrawColorArrow(BtnBmp, FCommonData.SkinManager.gd[iSkinIndex].Props[i].FontColor.Color, R, asBottom);
        end;
    end;
  end;
