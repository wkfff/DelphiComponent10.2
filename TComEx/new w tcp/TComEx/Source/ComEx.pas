...
//    procedure DecodeFlags(F:Integer);
    ...
    procedure OnConnectedClient(Sender: TObject);
    procedure OnDisconnectedClient(Sender: TObject);
    procedure OnSocketError(Sender: TObject);
...
FSocketModule.OnConnected := OnConnectedClient;
FSocketModule.OnDisconnected := OnDisconnectedClient;
FSocketModule.OnSocketError := OnSocketError;
...
try
 if FSocketModule <> NIL then
    try
     FSocketModule.ClientSocket1.Active := False;
     FSocketModule.ServerSocket1.Active := False;
    finally
...
procedure TComEx.OnConnectedClient(Sender: TObject);
begin
FIsOpen := True;
ComH := 1;
if FLogOnMemo AND (FMemo<>NIL) then
  try
    FMemo.Lines.Add('Connected to TCP Port : ' + inttostr(FNetPort));
  finally
  end;
end;

procedure TComEx.OnDisconnectedClient(Sender: TObject);
begin
FIsOpen := False;
ComH := 0;
if FLogOnMemo AND (FMemo<>NIL) then
  try
    FMemo.Lines.Add('Disconnected from TCP Port : ' + inttostr(FNetPort));
  finally
  end;
end;

procedure TComEx.OnSocketError(Sender: TObject);
var ls: TFormatSettings;
begin
  {$IF CompilerVersion<22}
  GetLocaleFormatSettings(-1,ls);
  {$ELSE}
  ...
  if FLogOnMemo AND (FMemo <> NIL) then
    try
      FMemo.Lines.Add(TimeToStr(Time,ls)+Format(' Socket Error (%d)',[FSocketModule.ErrorCode]));
    finally
    end;
  if FLogOnFile then
    try
      SaveFileLog(TimeToStr(Time,ls)+Format(' Socket Error (%d)',[FSocketModule.ErrorCode]));
    finally
    end;
end;
...
case FUseNet of
 ...
 nmMTCPIP:begin
           try
              FSocketModule.ServerSocket1.Port := FNetPort;
              FSocketModule.ServerSocket1.Active := True;
              Result:=True;
           except
              Result:=False;
           end;
 ...
 nmSTCPIP:begin
           try
              FSocketModule.ClientSocket1.Host := FNetFile;
              FSocketModule.ClientSocket1.Port := FNetPort;
              FSocketModule.ClientSocket1.Active := True;
              Result:=True;
           except
              Result:=False;
           end;
...
//Per le connessioni su socket devo aspettare.
if (FUseNet <> nmMTCPIP) and (FUseNet <> nmSTCPIP) then
  begin
    FIsOpen:=Result;
    if FLogOnMemo AND (FMemo<>NIL) then
       try
  ...
  begin
    if FLogOnMemo AND (FMemo<>NIL) then
      try
        if (FUseNet = nmMTCPIP) then
          FMemo.Lines.Add('Waiting for connections on TCP Port : ' + inttostr(FNetPort))
        else
          FMemo.Lines.Add('Connecting to TCP server : '+ FNetFile + ' Port : ' + inttostr(FNetPort));
      finally
      end;
  end;
...
if (ComH>0) AND (ComH<>INVALID_HANDLE_VALUE) then
   try
    case FUseNet of
     nmMASTER,
     nmSLAVE:;
     nmMTCPIP:FSocketModule.ServerSocket1.Active:=False;
     nmSTCPIP:FSocketModule.ClientSocket1.Active:=False;
...
case FUseNet of
 ...
 nmSTCPIP:
   begin
      if FUseOverlap then
        FSocketModule.SocketStream1.TimeOut := TimeOuts.ReadIntervalTimeout;
      ReadBytes := FSocketModule.SocketStream1.Read(Buffer,Count);
      if ReadBytes <> 0 then
        r:=True;
   end;
 nmNone:
...
if r then
   begin
    ...
    if FLogOnFile AND (ReadBytes<>0) then
       try
        SaveFileLog(TimeToStr(Time,ls)+Format(': Rx: %s [B read %d]',[s,ReadBytes]));
       finally
       end;
   end;
end;
...
case FUseNet of
 ...
 nmSTCPIP:
   begin
      if FUseOverlap then
        FSocketModule.SocketStream1.TimeOut := TimeOuts.WriteTotalTimeoutConstant;
      WrittenBytes := FSocketModule.SocketStream1.Write(PLinea^,Count);
...
case FUseNet of
 ...
 nmSTCPIP:
   begin
      if FUseOverlap then
        FSocketModule.SocketStream1.TimeOut := TimeOuts.WriteTotalTimeoutConstant;
      WrittenBytes := FSocketModule.SocketStream1.Write(Buffer,Count);
      if WrittenBytes <> 0 then
        Result:=True;
   end;
 nmNone:
...
procedure TComEx.SetMemo(Value: TMemo);
begin
FMemo:=Value;
end;
